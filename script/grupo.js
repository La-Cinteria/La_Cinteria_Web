document.addEventListener("DOMContentLoaded", () => {
    ...
    .then(data => {
        ...
        Object.values(productoActivo.fotos).forEach(listaFotos => {
    ...
    function actualizarUI() {
        ...
        // IMAGEN
        slides.innerHTML = `<img src="${productoActivo.fotos[colorSeleccionado][0]}" class="slide active">`;
    ...
    function anteriorFoto() {
        const colores = productoActivo.colores;
        let indice = colores.findIndex(c => c.nombre === colorSeleccionado);

        indice = (indice - 1 + colores.length) % colores.length;
        colorSeleccionado = colores[indice].nombre;
    ...
    // ── Swipe en celular y drag con mouse ─────────────────────────
    let startX = 0;
    let endX = 0;
    let dragging = false;
    ...
    function procesarDeslizamiento() {
        const diferencia = endX - startX;

        // Evita cambios por movimientos muy pequeños
    ...
    window.anteriorFoto = anteriorFoto;
